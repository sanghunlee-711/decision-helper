...
export default function MenuClient({
  ...
  /** ✅ 새로운 카테고리 추가 */
  const addCategory = () => {
    if (!categoryText.trim()) return showAlert("카테고리 이름을 입력하세요.");
  ...
  const addSubcategory = (categoryId: string) => {
    if (!subcategoryText[categoryId]?.trim())
      return showAlert("서브카테고리 이름을 입력하세요.");
    setMenus((prev) => ({
      categories: prev.categories.map((cat) =>
        cat.id === categoryId
          ? {
              ...
              subcategories: [
                ...(cat.subcategories || []),
                { id: generateId(), name: subcategoryText[categoryId] },
              ],
            }
          : cat
      ),
                ...
                <AccordionContent>
                  ...
                  <ul className="ml-4 mt-4 space-y-2">
                    {(category.subcategories || []).map((sub) => (
                      <li
                        key={sub.id}
                        className="text-gray-700 text-sm px-2 py-1 rounded bg-gray-100 inline-block"
                      >
                        {sub.name}
                      </li>
                    ))}
