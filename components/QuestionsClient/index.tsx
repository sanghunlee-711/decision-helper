...
export default function QuestionsClient({ initialMenus, email }: { initialMenus: MenuData; email: string }) {
  ...
  const addQuestion = (categoryId: string) => {
    if (!questionText[categoryId]?.trim()) return alert("질문을 입력하세요.");
    ...
    const addAnswer = (categoryId: string, questionId: string) => {
      if (!answerText[questionId]?.trim()) return alert("답변을 입력하세요.");
  ...
  const addWeight = (categoryId: string, questionId: string, answerId: string) => {
    if (!selectedSubcategory[answerId]) return alert("서브카테고리를 선택하세요.");
                          ...
                          ? {
                              ...
                              weights: [
                                ...ans.weights,
                                {
                                  ...
                                  name:
                                    cat.subcategories.find((sub) => sub.id === selectedSubcategory[answerId])
                                      ?.name || "Unknown",
                                  weight: 1, // 기본 가중치 1
                                },
                              ],
                        ...
                        ans.id === answerId
                          ? {
                              ...
                              weights: ans.weights.map((w) =>
                                w.id === subcategoryId ? { ...w, weight: value } : w
                              ),
                            }
                          : ans
                      ),
                ...
                <AccordionContent>
                  ...
                  <div className="flex gap-2 mt-2">
                    <Input
                      ...
                      onChange={(e) => setQuestionText((prev) => ({ ...prev, [category.id]: e.target.value }))}
                      placeholder="새 질문 입력"
                    />
                    <Button onClick={() => addQuestion(category.id)} variant="outline">
                      질문 추가
                    </Button>
                  </div>
                  ...
                  {category.questions?.map((question) => (
                    <div key={question.id} className="ml-4 border-l pl-4 mt-4">
                      <strong className="block text-lg font-semibold">{question.question}</strong>
                      ...
                      <div className="flex gap-2 mt-2">
                        <Input
                          ...
                          placeholder="새 답변 입력"
                        />
                        <Button onClick={() => addAnswer(category.id, question.id)}>답변 추가</Button>
                      </div>
                      ...
                      {question.answers.map((answer) => (
                        <div key={answer.id} className="ml-4 mt-4 border-l pl-4">
                          ...
                          <div className="flex gap-2 mt-2">
                            ...
                            <Button onClick={() => addWeight(category.id, question.id, answer.id)}>가중치 추가</Button>
                          </div>

                          {answer.weights.map((w) => (
                            <div key={w.id} className="flex items-center gap-2 mt-2">
                              <span>{w.name}</span>
                              <Slider
                                min={1}
                                max={5}
